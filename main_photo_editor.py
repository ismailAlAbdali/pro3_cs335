    ...
    def changeContrast(self, contrast):
        ...
        for row_pixel in range(self.image.width()):
            for col_pixel in range(self.image.height()):
                ...
                current_val = QColor(self.image.pixel(row_pixel, col_pixel))
                red = self.clamp(factor * (current_val.red() - 128) + 128)
                green = self.clamp(factor * (current_val.green() - 128) + 128)
                blue = self.clamp(factor * (current_val.blue() - 128) + 128)
...
class PhotoEditorGUI(QMainWindow):
    ...
    def createMenu(self):
        ...
        ## remove those zoom in and zoom out buttons
